...
用hashmap和double linked list\(DDL\)做。两个hashmap一个存\(key, node\)，一个存\(freq, DDL\)， 因为DDL可以每次都在头部添加，天然的做到least frequent放在tail，如果删除就移除tail。 每次调用get都要更新freq，根据freq放在新的DDL里面，并且在旧的DDL里面移除这个node.
