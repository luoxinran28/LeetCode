...
用一个map作为领结表存储key还有对应的node，用双向链表链接所有的node，双向 链表便于进行删除结点插入头结点等操作。用一个head和一个tail指针在以前以后， 可以便于操作。
 ...
 */
/*
用一个map作为领结表存储key还有对应的node，用双向链表链接所有的node，双向
链表便于进行删除结点插入头结点等操作。用一个head和一个tail指针在以前以后，
可以便于操作。
*/
